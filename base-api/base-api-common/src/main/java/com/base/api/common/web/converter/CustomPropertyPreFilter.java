...
import com.base.api.common.annotation.DateFormatAnnotation;
		...
		if(field != null){
			...
			if(annArray != null && annArray.length > 0){
				for(Annotation an : annArray) {
					...
					if (annotation.equals(DateFormatAnnotation.class)) {
						DateFormatAnnotation an2 = (DateFormatAnnotation) an;
						value = DateUtils.getStringDate((Date)value,an2.format());
					}
				}
			}else{
